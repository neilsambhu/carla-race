...
class CarEnv:
    ...
    def reset(self):
        ...
        self.rgb.set_attribute("image_size_x", f"{self.im_height}")
...
class DQNAgent:
    ...
    def train(self):
        ...
        with self.graph.as_default():
            current_qs_list = self.model.predit(current_states, PREDICTION_BATCH_SIZE)

        new_current_states = np.array([transition[3] for transition in minibatch])/255
        with self.graph.as_default():
            future_qs_list = self.target_model.predit(new_current_states, PREDICTION_BATCH_SIZE)
    ...
    def train_in_loop(self):
        ...
        y = np.random.uniform(size=(1, 3)).astype(np.flat32)
        with self.graph.as_default():
